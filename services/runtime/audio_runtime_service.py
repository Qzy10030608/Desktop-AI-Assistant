...
class AudioRuntimeService:
    ...
    def __init__(self, controller: Any):
        self.c = controller
        self.input_device_service = InputDeviceService()
    ...
    def handle_start_record(self):
        ...
        try:
            ...
            input_resolution = self.input_device_service.resolve_input_device()
            device_index = input_resolution.get("device_index")
            ...
            print(
                "[AudioInput] "
                f"resolved={input_resolution.get('resolved', False)!r} "
                f"fallback={input_resolution.get('fallback', False)!r} "
                f"device_index={device_index!r} "
                f"device_name={str(input_resolution.get('device_name', '') or '')!r} "
                f"reason={str(input_resolution.get('reason', '') or '')!r}"
            )
            ...
            try:
                ...
                print(
                    "[AudioInput] selected_device_failed "
                    f"device_index={device_index!r} "
                    f"fallback=True "
                    f"error={rec_exc!r}"
                )
    ...
    def handle_stop_record(self):
        ...
        try:
            ...
            buffered_frames = int(len(self.c.record_buffer)) if hasattr(self.c.record_buffer, "__len__") else 0
            valid_frames = max(0, min(int(elapsed * SAMPLE_RATE), buffered_frames))
            valid_audio = self.c.record_buffer[:valid_frames]
            valid_audio = self._prepare_record_audio_for_asr(valid_audio)

            abs_audio = np.abs(valid_audio) if valid_audio.size > 0 else np.asarray([], dtype=np.float32)
            peak = float(np.max(abs_audio)) if abs_audio.size > 0 else 0.0
            mean_abs = float(np.mean(abs_audio)) if abs_audio.size > 0 else 0.0
            rms = float(np.sqrt(np.mean(np.square(valid_audio)))) if valid_audio.size > 0 else 0.0
            is_silent = peak < 0.01 and rms < 0.003
            ...
            file_size = os.path.getsize(record_path) if os.path.exists(record_path) else 0
            print(
                "[RecordDiag] "
                f"elapsed={elapsed:.2f} "
                f"valid_frames={valid_frames} "
                f"audio_shape={tuple(valid_audio.shape)} "
                f"max_abs={peak:.4f} "
                f"mean_abs={mean_abs:.4f} "
                f"rms={rms:.4f} "
                f"is_silent={is_silent!r} "
            ...
            if is_silent:
                ...
                if hasattr(self.c.window, "set_latest_record_recognized_status"):
                    self.c.window.set_latest_record_recognized_status("未检测到有效麦克风输入")
                self.c.window.set_status("没有检测到有效麦克风输入，请检查输入设备。")
                if hasattr(self.c.window, "append_system_message"):
                    self.c.window.append_system_message(
                        "没有检测到有效麦克风输入，请回放录音条确认，或在基础设置中切换输入设备。"
                    )
                if hasattr(self.c.window, "set_asr_runtime_status"):
    ...
    def on_asr_finished(self, request_id: int, recognized_text: str):
        ...
        if not self.c.current_recognized_text:
            if hasattr(self.c.window, "set_latest_record_recognized_status"):
                self.c.window.set_latest_record_recognized_status("未识别到有效语音")
    ...
    def on_asr_error(self, request_id: int, msg: str):
        ...
        if hasattr(self.c.window, "set_latest_record_recognized_status"):
            self.c.window.set_latest_record_recognized_status("语音识别失败")
