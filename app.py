...
class StartupTTSPrepareWorker(QObject):
    ...
    def run(self):
        try:
            result = self.tts_backend_controller.prepare_current_runtime(
...
class DesktopAIController(QObject):
    ...
    def __init__(self):
        ...
        self.yushitai_report_writer = ReportWriter(self.machine_profile_service.project_root)
        self.yushitai_run_id = self._start_yushitai_run()
    ...
    def _start_yushitai_run(self) -> str:
        try:
            self.yushitai_report_writer.configure_session(
                run_scope="app_session",
                started_by="app_startup",
            )
            meta = self.yushitai_report_writer.start_run(
                run_backend="host",
            ...
            print(f"[YushitaiStartup] start run failed: {exc!r}")
            return ""

    def close_yushitai_run(self) -> None:
        current_mode = ""
        test_backend = "sandbox"
        try:
            control_center = getattr(self, "control_center_window", None)
            runtime = getattr(control_center, "desktop_runtime", None)
            if runtime is not None:
                test_backend = str(getattr(runtime, "test_backend", "sandbox") or "sandbox")
                current_mode = str(runtime.qin_runtime.mode_store.get_mode_state().current_mode or "")
        except Exception:
            pass
        try:
            self.yushitai_report_writer.close_session_runs(
    ...
    def bind_signals(self):
        ...
        self.window.ui_language_changed.connect(self.handle_ui_language_changed)
    ...
    def handle_ui_language_changed(self, _locale: str):
        if self.control_center_window is not None and hasattr(self.control_center_window, "retranslate_ui"):
    ...
    def _append_pending_interaction_card_compat(self, ui_prompt: dict):
        prompt = ui_prompt if isinstance(ui_prompt, dict) else {}
        if hasattr(self.window, "append_pending_interaction_card"):
            widget = self.window.append_pending_interaction_card(prompt)
            if widget is not None and hasattr(widget, "action_requested"):
                try:
                    widget.action_requested.disconnect()
                except Exception:
                    pass
                widget.action_requested.connect(self.handle_pending_ui_action)
            return widget

        display_text = str(prompt.get("display_text", "") or "").strip()
        if display_text:
            self._append_ai_text_only_message(display_text)
        return None

    def handle_pending_ui_action(self, event: dict):
        payload = event if isinstance(event, dict) else {}
        print("[PendingInteractionCard] action event=", payload)
        self.window.set_status("正在处理你的确认...")
        ...
        if action not in {"cancel", "confirm", "select_candidate"}:
            return

        result = self.chat_runtime_service.handle_pending_ui_action(payload)
        result_payload = result if isinstance(result, dict) else {}
        message = str(result_payload.get("safe_user_message", "") or "").strip()
        if not message:
            message = str(result_payload.get("message", "") or result_payload.get("error", "") or "桌面操作未能完成。")

        if action == "cancel":
            if hasattr(self.window, "remove_pending_interaction_card"):
                try:
                    self.window.remove_pending_interaction_card(pending_task_id)
                except Exception as exc:
                    print(f"[PendingInteractionCard] remove_failed pending_task_id={pending_task_id!r} error={exc!r}")
            if message:
                self.window.set_status(message)
                self._append_ai_text_only_message(message)
            return
        ...
        if message:
            self.window.set_status(message)
            self._append_ai_text_only_message(message)
        if action in {"confirm", "select_candidate"}:
            return
    ...
    def handle_send_text(self, text: str):
        ...
        print(f"[InputRoute] text input forwarded to ChatRuntimeService text={text[:40]!r}")
    ...
    def handle_open_control_center(self):
        ...
        if not getattr(self.control_center_window, "_initial_state_loaded", False):
            self.control_center_window.load_current_state()  # type: ignore
        else:
            output_combo = getattr(self.control_center_window, "output_mode_combo", None)
            if output_combo is not None:
                output_combo.blockSignals(True)
                idx = output_combo.findData(self.current_output_mode)
                if idx >= 0:
