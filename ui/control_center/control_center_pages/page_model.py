...
def _build_status_dot(window) -> QLabel:
    dot = QLabel("●")
    dot.setFixedWidth(window.UI_SIZE["status_dot_width"])
    dot.setStyleSheet(
...
def build_model_page(window) -> QFrame:
    ...
    window.summary_desktop_mode_label = QLabel("桌面模式：-")
    window.summary_developer_mode_label = QLabel("开发者模式：开启")
    window.summary_execution_backend_label = QLabel("执行出口：None")
    window.summary_developer_mode_label.setVisible(False)
    window.summary_execution_backend_label.setVisible(False)
    ...
    summary_layout.addWidget(window.summary_desktop_mode_label)
    summary_layout.addWidget(window.summary_developer_mode_label)
    summary_layout.addWidget(window.summary_execution_backend_label)
    ...
    # -------- 语音模型 --------
    window.tts_backend_status_dot = _build_status_dot(window)
    ...
    gif_size = window.UI_SIZE["tts_loading_gif_size"]
    window.tts_loading_gif_label.setFixedSize(gif_size, gif_size)
    ...
    window.current_scheme_display.setFixedHeight(window.UI_SIZE["scheme_display_height"])
