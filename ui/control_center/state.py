...
class ControlCenterState:
    ...
    def page_key_from_index(self, index: int) -> str:
        mapping = {
            ...
            3: "connection",  # 连接配置
            4: "desktop",     # 桌面连接
            5: "basic_settings",  # 基础设置
        }
        return mapping.get(index, "model")

    def _is_dirty_managed_page(self, page_key: str) -> bool:
    ...
    def build_style_snapshot(self) -> dict:
        style_data = (
            self.w.current_style_config_combo.currentData()
            if getattr(self.w, "current_style_config_combo", None)
            else {}
        )
        voice_data = (
            self.w.current_voice_config_combo.currentData()
            if getattr(self.w, "current_voice_config_combo", None)
            else {}
        )
    ...
    def set_dirty(self, page_key: str, dirty: bool):
        ...
        if not self._is_dirty_managed_page(page_key):
            return

        self.w.page_dirty[page_key] = bool(dirty)
    ...
    def capture_all_snapshots(self):
        ...
        # 其他页统一清零
        for page_key in ("model", "connection", "desktop"):
            if page_key in self.w.page_dirty:
                self.w.page_dirty[page_key] = False
    ...
    def update_window_title(self):
        ...
        if self._is_dirty_managed_page(current_key) and self.w.page_dirty.get(current_key, False):
    ...
    def sync_nav_buttons(self):
        ...
        if getattr(self.w, "btn_model", None) is not None:
            self.w.btn_model.setChecked(index == 0)
        if getattr(self.w, "btn_style", None) is not None:
            self.w.btn_style.setChecked(index == 1)
        if getattr(self.w, "btn_info", None) is not None:
            self.w.btn_info.setChecked(index == 2)
        if getattr(self.w, "btn_connection", None) is not None:
            self.w.btn_connection.setChecked(index == 3)
        if getattr(self.w, "btn_desktop", None) is not None:
            self.w.btn_desktop.setChecked(index == 4)
        if getattr(self.w, "btn_basic_settings", None) is not None:
            self.w.btn_basic_settings.setChecked(index == 5)
    ...
    def close_event(self, event):
        dirty_pages = [
            page_key
            for page_key in ("style", "info")
            if self.w.page_dirty.get(page_key, False)
        ]
        ...
        dirty_names = [page_name_map.get(k, k) for k in dirty_pages]
