...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(self.tr_ui("app.title"))
        ...
        self.assistant_display_name = self.chat_display_config_service.get_assistant_display_name()
    ...
    def init_ui(self):
        ...
        self.chat_panel = ChatPanel(assistant_display_name=self.assistant_display_name)
        ...
        self.settings_popup.language_selected.connect(self.on_language_selected)

        self.status_popup.set_status_data(self.runtime_state_data)
        self.settings_popup.set_current_mode(self.current_output_mode)
        self.settings_popup.set_current_language(self.ui_language_service.locale)
        self.retranslate_ui()
    ...
    def retranslate_ui(self):
        ...
        if hasattr(self, "top_toolbar"):
            self.top_toolbar.retranslate_ui(self.tr_ui)
        if hasattr(self, "input_label"):
            self.input_label.setText(self.tr_ui("main.input_label"))
        if hasattr(self, "text_input"):
            self.text_input.setPlaceholderText(self.tr_ui("main.input_placeholder"))
        if hasattr(self, "send_text_btn"):
            self.send_text_btn.setText(self.tr_ui("main.send_text"))
        if hasattr(self, "start_record_btn"):
            self.start_record_btn.setText(self.tr_ui("main.start_record"))
        if hasattr(self, "stop_record_btn"):
            self.stop_record_btn.setText(self.tr_ui("main.stop_record"))
        if hasattr(self, "settings_popup"):
            self.settings_popup.retranslate_ui(self.tr_ui, self.ui_language_service.locale)
        if hasattr(self, "runtime_state_data"):
            self.runtime_state_data["output_mode"] = self.get_output_mode_display_name(self.current_output_mode)
        if hasattr(self, "status_popup"):
            self.status_popup.set_status_data(self.runtime_state_data)

    def on_language_selected(self, locale: str):
        self.ui_language_service.set_locale(locale)
        self.retranslate_ui()
        if hasattr(self, "control_center_window") and self.control_center_window is not None:
            if hasattr(self.control_center_window, "retranslate_ui"):
                self.control_center_window.retranslate_ui()
        self.ui_language_changed.emit(locale)
        self._show_current_output_mode_hint()
    ...
    def _show_current_output_mode_hint(self):
        self.show_hint_bar(
            self.tr_ui(
                "main.output_mode_hint",
                mode=self.get_output_mode_display_name(self.current_output_mode),
            ),
            1600
        )
    ...
    def append_pending_interaction_card(self, ui_prompt: dict):
        if hasattr(self.chat_panel, "append_pending_interaction_card"):
            return self.chat_panel.append_pending_interaction_card(ui_prompt)
        return None

    def remove_pending_interaction_card(self, pending_task_id: str) -> None:
        if hasattr(self.chat_panel, "remove_pending_interaction_card"):
            self.chat_panel.remove_pending_interaction_card(pending_task_id)

    def mark_pending_interaction_resolved(self, pending_task_id: str, text: str = "") -> None:
        if hasattr(self.chat_panel, "mark_pending_interaction_resolved"):
            self.chat_panel.mark_pending_interaction_resolved(pending_task_id, text)

    def mark_pending_interaction_cancelled(self, pending_task_id: str, text: str = "") -> None:
        if hasattr(self.chat_panel, "mark_pending_interaction_cancelled"):
            self.chat_panel.mark_pending_interaction_cancelled(pending_task_id, text)
    ...
    def get_output_mode_display_name(self, mode: str) -> str:
        return {
            "text_only": self.tr_ui("output.text_only"),
            "text_voice": self.tr_ui("output.text_voice"),
            "voice_only": self.tr_ui("output.voice_only"),
        }.get(mode, self.tr_ui("output.text_voice"))

    def on_output_mode_selected(self, mode: str):
        self.set_output_mode(mode)
        self.output_mode_changed.emit(mode)
        self.show_hint_bar(
            self.tr_ui("main.output_mode_hint", mode=self.get_output_mode_display_name(mode)),
            1600,
        )
    ...
    def set_recognized_text(self, text: str):
        ...
        if self.current_recognized_text:
            updated = False
            if hasattr(self.chat_panel, "update_latest_record_recognized_text"):
                updated = self.chat_panel.update_latest_record_recognized_text(
                    self.current_recognized_text
                )
            if not updated:
            ...
            if hasattr(self.chat_panel, "update_latest_record_recognized_status"):
                self.chat_panel.update_latest_record_recognized_status("未识别到有效语音")
            self.set_status("未识别到有效语音内容")

    def set_latest_record_recognized_status(self, text: str) -> bool:
        if hasattr(self.chat_panel, "update_latest_record_recognized_status"):
            return bool(self.chat_panel.update_latest_record_recognized_status(text))
        return False

    def set_assistant_display_name(self, name: str):
        self.assistant_display_name = str(name or "").strip() or "AI"
        if hasattr(self.chat_panel, "set_assistant_display_name"):
            self.chat_panel.set_assistant_display_name(self.assistant_display_name)
